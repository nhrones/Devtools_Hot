...
```json
   "hot": "deno run --allow-all https://jsr.io/@ndh/hot/1.0.17/server.ts"
```
...
```json
      {
         "label": "HOT",
         "type": "shell",
         "command": "deno run --allow-all --no-config https://jsr.io/@ndh/hot/1.0.17/server.ts .",
         "problemMatcher": [],
         "group": {
            "kind": "build",
            "isDefault": true
...
``` 
deno run --allow-all --no-config https://jsr.io/@ndh/hot/1.0.17/server.ts .
```
